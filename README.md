...
需要使用时，将本仓库的 `skills/luna-keeper` 整个目录安装到宿主的 Skill 管理位置。宿主必须提供相应的跨任务接口；缺失的能力会明确报告。示例：

```text
使用 $luna-keeper，依据刚才的 Luna Roadmap 方案新建独立执行任务。
执行协调者使用 gpt-5.6-terra，子 Agent 按方案选择模型。
自动收取执行结果，在已授权范围内制定并发送后续指令，
直到整体目标完成或遇到需要我决策的问题。
```

也可以指定已有执行任务继续跟进。默认在当前规划回合内接续；只有明确要求后台跟进时才检查并配置宿主自动化，Skill 本身不提供常驻运行能力。新 worktree 的输入可见性、实际执行模型和验收证据均需在执行时核实。
