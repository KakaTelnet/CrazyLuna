...
## 配套编排 Skill：Luna Keeper

[Luna Keeper](skills/luna-keeper/SKILL.md) 在 Codex 的规划会话中衔接执行任务：创建或续接独立任务、发送交接方案、等待执行结果，再依据验收证据推进下一轮。Luna Roadmap 负责方案与执行协议，执行任务负责子 Agent 派发、实现和集成。

需要使用时，将本仓库的 `skills/luna-keeper` 整个目录安装到宿主的 Skill 管理位置。宿主必须提供相应的跨任务接口；缺失的能力会明确报告。在规划对话 A 中讨论清楚目标或修改点后，输入：

```text
使用 $luna-keeper 执行任务，完成上述目标。
```

Keeper 从 A 的上下文接收目标，使用 Luna Roadmap 制定方案与 Prompt，按授权启动执行对话 B；A 自动收取 B 的结果、核对验收证据，并让同一个 B 继续处理剩余工作，直到整体目标完成。已有适用方案或执行任务时直接复用，用户无需手动复制交接内容。

新建 B 及宿主要求的模型授权沿用用户已有明确安排；首次缺少必要授权时，Keeper 准备好具体安排后简短确认一次。默认在当前规划回合内接续；只有明确要求后台跟进时才检查并配置宿主自动化，Skill 本身不提供常驻运行能力。新 worktree 的输入可见性、实际执行模型和验收证据均需在执行时核实。
