...
module CrazyLunaValidation
  ...
  def self.validate(documents)
    ...
    check(front['name'] == 'crazy-luna', 'frontmatter: name must be crazy-luna')
    ...
    check(prompt.is_a?(String) && prompt.include?('$crazy-luna'), 'UI: default_prompt must name $crazy-luna')
    ...
    check((config.keys - %w[allowed_models default_model reasoning_preference]).empty?, 'models: unsupported configuration field')
    allowed = config['allowed_models']
    check(allowed.is_a?(Array) && !allowed.empty? &&
          allowed.all? { |id| id.is_a?(String) && !id.strip.empty? }, 'models: allowed_models must contain nonempty IDs')
    check(config['default_model'] == 'auto' || allowed.include?(config['default_model']),
          'models: fixed default must belong to allowed_models')
    if config.key?('reasoning_preference')
      preference = config['reasoning_preference']
      check(preference.is_a?(String) && !preference.strip.empty?, 'models: reasoning_preference must be a nonempty string')
    end
...
if $PROGRAM_NAME == __FILE__
  begin
    raise CrazyLunaValidation::Invalid, 'Usage: ruby validate.rb [skill-directory]' if ARGV.length > 1
    ...
    CrazyLunaValidation.validate(documents)
    puts 'PASS: crazy-luna portable package profile (static only)'
    puts 'Host loading, model availability and task execution are not verified by this check.'
  rescue CrazyLunaValidation::Invalid, SystemCallError => e
    warn "FAIL: #{e.message}"
    exit 1
  end
end
