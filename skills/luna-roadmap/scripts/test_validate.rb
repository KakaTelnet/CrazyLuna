#!/usr/bin/env ruby
# Run the Luna Roadmap package validator against role-model configuration and
...
cases.each do |id, name, expectation, builder|
  begin
    ...
    if expectation == :accept
      result = LunaRoadmapValidation.validate(built)
      raise "expected true, got #{result.inspect}" unless result == true
    else
      begin
        result = LunaRoadmapValidation.validate(built)
        raise "expected LunaRoadmapValidation::Invalid, got #{result.inspect}"
      rescue LunaRoadmapValidation::Invalid => error
        expected_fragments = {
