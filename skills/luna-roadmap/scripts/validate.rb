...
module LunaRoadmapValidation
  ...
  def self.validate(documents)
    ...
    check(front['name'] == 'luna-roadmap', 'frontmatter: name must be luna-roadmap')
    ...
    check(prompt.is_a?(String) && prompt.include?('$luna-roadmap'), 'UI: default_prompt must name $luna-roadmap')
...
if $PROGRAM_NAME == __FILE__
  begin
    raise LunaRoadmapValidation::Invalid, 'Usage: ruby validate.rb [skill-directory]' if ARGV.length > 1
    ...
    LunaRoadmapValidation.validate(documents)
    puts 'PASS: luna-roadmap portable package profile (static only)'
    puts 'Host loading, model availability and task execution are not verified by this check.'
  rescue LunaRoadmapValidation::Invalid, SystemCallError => e
    warn "FAIL: #{e.message}"
    exit 1
  end
end
